  ...
  describe("ESP3 Parser", function () {
    ...
    describe("ESP3 Packets of type:",function(){
      it("SMART_ACK_COMMAND SHOULD be emitted as SmartAckCommand Object", function () {
        ...
        assert.equal(spy.args[0][0].constructor.name, "SmartAckCommand", "Packet not a Smart_Ack_Packet")
      })
      it("COMMON_COMMAND SHOULD be emitted as CommonCommand Object", function () {
        const spy = sinon.spy()
        esp3parser.on("data", spy)
        esp3parser.write(Buffer.from("5500010005700838","hex"))
        assert.equal(spy.args[0][0].constructor.name, "CommonCommand", "Packet not a Smart_Ack_Packet")
      })
      it("RADIO_ERP1 SHOULD be emitted as Radio_ERP1 Object", function () {
        ...
        assert.equal(spy.args[0][0].constructor.name, "RadioERP1", "Packet not a Smart_Ack_Packet")
      })
