...
describe('serialport enocean parser', function () {
  ...
  describe('ESP3 Parser', function () {
    ...
    describe("ESP3 Packets of type:",function(){
      it(`Smart_Ack_Command SHOULD be emitted as Smart_Ack_Command Object`, function () {
        const spy = sinon.spy();
        esp3parser.on('data', spy);
        esp3parser.write(Buffer.from("55000700060401010000000500b7","hex"))
        assert.equal(spy.args[0][0].constructor.name, "Smart_Ack_Command", `Packet not a Smart_Ack_Packet`);
      });
      it(`Common_Command SHOULD be emitted as Common_Command Object`, function () {
        const spy = sinon.spy();
        esp3parser.on('data', spy);
        esp3parser.write(Buffer.from("5500010005700838","hex"))
        assert.equal(spy.args[0][0].constructor.name, "Common_Command", `Packet not a Smart_Ack_Packet`);
      });
      it(`Radio_ERP1 SHOULD be emitted as Radio_ERP1 Object`, function () {
        const spy = sinon.spy();
        esp3parser.on('data', spy);
        esp3parser.write(Buffer.from("55000a0701eba5c87f710fffdba5e40001ffffffff47000d","hex"))
        assert.equal(spy.args[0][0].constructor.name, "Radio_ERP1", `Packet not a Smart_Ack_Packet`);
      });
